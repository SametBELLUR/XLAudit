...
// "B" → 1, "AA" → 27. (1-indexed.) Sadece dahili kullanım.
...
// (tipik fill-down kullanımında okunabilirlik için). Satır relative ise
// {row}/{row±N}, mutlak ise $row literal.
function formatCellPattern(cell, anchor) {
  ...
  if (cell.absRow) {
    ...
    else rowPart = `{row${offset}}`; // negatif değer kendisi - işaretini taşır
  }
  return colPart + rowPart;
}

// Token listesinden anchor bazlı patern stringi üretir.
// Yardımcı bilgileri de döner: numericConstants, sheetRefs, extRefs,
// hasRelativeRow (en az bir relative-row içeren CELL var mı).
export function patternize(tokens, anchor) {
...
// Formül listesini patern'e göre gruplar.
// Dönüş: Map<pattern, { pattern, cells, numericConstants, sample,
//                       sheetRefs, extRefs, hasRelativeRow }>
export function groupByPattern(formulas) {
  ...
  // Set → array (UI'da iterate kolay olsun)
...
// [{addr, row, col, colLetter}, ...] → ['C2:C100', 'D5', ...]
// Aynı sütundaki ardışık satırları aralık olarak sıkıştırır.
export function compactRanges(cells) {
...
// Test/debug yardımcısı — DevTools'tan direk import edip çağırılabilir.
